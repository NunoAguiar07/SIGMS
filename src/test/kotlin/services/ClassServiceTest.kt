...
class ClassServiceTest {
    ...
    fun `getAllClassesFromSubject returns classes with default parameters`() {
        ...
        val result = classService.getAllClassesFromSubject(subject[0].id.toString(), null, null)
    ...
    @Test
    fun `getAllClassesFromSubject fails with invalid limit`() {
        val subject = createTestSubjects()
        assertNotNull(subject[0], "Subject should not be null")
        val classes = createTestClasses(5, subject[0])
        assertNotNull(classes, "Classes should not be null")
        assertNotNull(subject[0].id, "Subject ID should not be null")
        val result = classService.getAllClassesFromSubject(subject[0].id.toString(), "200", null)
        assertTrue(result is Failure, "Expected Failure")
        assertEquals(
            ClassError.InvalidClassLimit,
            result.value,
            "Expected InvalidClassLimit error"
        )
    }

    @Test
    fun `getAllClassesFromSubject fails with invalid offset`() {
        val subject = createTestSubjects()
        assertNotNull(subject[0], "Subject should not be null")
        val classes = createTestClasses(5, subject[0])
        assertNotNull(classes, "Classes should not be null")
        assertNotNull(subject[0].id, "Subject ID should not be null")
        val result = classService.getAllClassesFromSubject(subject[0].id.toString(), null, "-1")
        assertTrue(result is Failure, "Expected Failure")
        assertEquals(
            ClassError.InvalidClassOffset,
            result.value,
            "Expected InvalidClassOffset error"
        )
    }

    @Test
    fun `getAllClassesFromSubject fails with invalid subject ID`() {
        val result = classService.getAllClassesFromSubject("invalid", null, null)
        assertTrue(result is Failure, "Expected Failure")
        assertEquals(
            ClassError.InvalidSubjectId,
            result.value,
            "Expected InvalidSubjectId error"
        )
    }

    @Test
    fun `getAllClassesFromSubject fails with non-existing subject ID`() {
        val result = classService.getAllClassesFromSubject("999", null, null)
        assertTrue(result is Failure, "Expected Failure")
        assertEquals(
    ...
    fun `getClassById returns class when exists`() {
        ...
        assertNotNull(classes, "Classes should not be null")
        assertNotNull(subject[0].id, "Subject ID should not be null")
        val result = classService.getClassById(classes[0].id.toString())
        assertTrue(result is Success, "Expected Success")
        val successResult = result.value
    ...
    @Test
    fun `getClassById fails with invalid class ID`() {
        val result = classService.getClassById(null)
        assertTrue(result is Failure, "Expected Failure")
        assertEquals(
            ClassError.InvalidClassData,
            result.value,
            "Expected InvalidClassData error"
        )
    }
    ...
    fun `createClass succeeds with valid parameters`() {
        val subject = createTestSubjects()
        assertNotNull(subject[0], "Subject should not be null")
        val result = classService.createClass("Test Class", subject[0].id.toString())
        assertTrue(result is Success, "Expected Success")
        val successResult = result.value
    ...
    @Test
    fun `createClass fails with blank name`() {
        val subject = createTestSubjects()
        assertNotNull(subject[0], "Subject should not be null")
        val result = classService.createClass("", subject[0].id.toString())
        assertTrue(result is Failure, "Expected Failure")
        assertEquals(
            ClassError.InvalidClassData,
            result.value,
    ...
    @Test
    fun `createClass fails with invalid subject ID`() {
        val result = classService.createClass("Test Class", "invalid")
        assertTrue(result is Failure, "Expected Failure")
        assertEquals(
            ClassError.InvalidSubjectId,
            result.value,
    ...
    fun `createClass fails with duplicate class name for same subject`() {
        val subject = createTestSubjects()
        assertNotNull(subject[0], "Subject should not be null")
        val class1 = classService.createClass("Test 1", subject[0].id.toString())
        assertTrue(class1 is Success, "Expected Success")
        val class2 = classService.createClass("Test 1", subject[0].id.toString())
