...
class AdminRepository(private val database: Database): AdminRepositoryInterface {
    override fun getAllAdminEmails(): List<String> = withDatabase {
        database.admins.mapNotNull { it.user.email }
    }

    override fun findAdminById(id: Int): Admin? = withDatabase {
       return database.admins.firstOrNull { it.user eq id }
    }

    override fun findAdminByEmail(email:String): Admin? = withDatabase {
        val user = database.users.firstOrNull { it.email eq email } ?: return null
        return database.admins.firstOrNull { it.user eq user.id }
    }

    override fun isAdmin(user: User): Boolean = withDatabase {
